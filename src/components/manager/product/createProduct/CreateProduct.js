...
function CreateProduct({ closeAddingProduct }) {
  ...
  console.log("images add product", images);
  ...
  // Gọi hàm updateCategoryData khi category hoặc subCategory thay đổi
  useEffect(() => {
    updateCategoryData();
  ...
  const handleSubmit = async () => {
    ...
    formData.append("brand", productBrName);
    formData.append("description", description);
    formData.append("quantity", quantity);
    formData.append("sellingType", sellingType);
    formData.append("categories", categoryData); // Assuming category is an array of IDs
    // formData.append("parentCategoryId", subCategory); // Assuming subCategory is an array of IDs
    formData.append("basePrice", MRRPPrice);
    ...
    if (shape.length > 0) {
      formData.append("shape.unitOfMass", shape.unitOfMass.label);
      formData.append("shape.unitOfLength", shape.unitOfLength.label);
      Object.entries(shape).forEach(([key, value]) => {
        if (typeof value !== "object" && value !== null) {
          formData.append(`shape.${key}`, value);
        }
      });
    }
    ...
    if (variants.length > 0) {
      variants.forEach((variant, index) => {
        ...
        formData.append(
          `productVariants[${index}].basePrice`,
          variant.basePrice
        );
        formData.append(
          `productVariants[${index}].discountPrice`,
        ...
        Object.entries(variant.optionValues).forEach(([key, value]) => {
          formData.append(
            `productVariants[${index}].attributes[${key}]`,
            value
          );
        });
        variant.images.forEach((image, imgIndex) => {
          formData.append(
            `productVariants[${index}].images[${imgIndex}]`,
            image
          );
        });
      });
            ...
            <div>
              ...
              {isOverlayVisible && (
                <AddVariant
                  ...
                  onHandleAddVariant={handleVariantChange}
                  onhandleAddOptions={handleAttributeChange}
                ></AddVariant>
              )}
            </div>
          ) : (
